...
export function readSourceEvaluation(runsDir: string, id: string): SourceEvaluation | null {
  ...
  try {
    ...
    if (!["cvefixes-source-pilot-v1", "cvefixes-source-pilot-v2", "cvefixes-source-pilot-v3"].includes(manifest?.suite) || manifest.grader !== "python-ast-reference-v1" || manifest.runtimeTests !== false

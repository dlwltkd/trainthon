...
import { STAGE_HINT, STAGE_LABEL, type RunView, type StageView } from "@/lib/derive";
...
export function StageRail({ view, now }: { view: RunView; now: number }) {
  const visible = view.stages.filter((s) => s.id !== "INIT");
  return (
  ...
  return (
    <li className={cn("flex min-w-[8.5rem] flex-1 items-start gap-2", !last && "pr-1")}>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="secondary truncate text-[0.78em] text-ink-3">{view.kind === "local_repository" && stage.id === "REPRODUCE" ? "Check the supplied regression" : view.kind === "local_repository" && stage.id === "REVIEW" ? "Review the test evidence" : STAGE_HINT[stage.id]}</div>
        {gate && (

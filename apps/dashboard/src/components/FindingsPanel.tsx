...
export function FindingsPanel({ view, files, onEvidence }: { view: RunView; files: string[]; onEvidence: (target: EvidenceTarget) => void }) {
  ...
  return <Panel className="overflow-hidden"><PanelHeader title={<><SearchCheck className="size-3.5" /> Findings</>} aside={redCount ? <span className="flex gap-1"><Chip tone="red">{pluralize(redCount, "Red claim")}</Chip><Chip tone="blue">{pluralize(blueCount, "Blue assessment")}</Chip></span> : <Chip>{view.findings.length}</Chip>} />

  ...
  it("hands Red findings to Blue and requires Blue's own source evidence before editing", async () => {
    ...
    const record = await executeRepositoryReview({
      ...
      reviewRunner: { run: async input => {
        ...
        expect(input.requestPolicy).toEqual({ maxRetries: 2, timeoutMs: 90_000, transport: "stream", progressEverySteps: 6 });
    ...
    expect(record.requestPolicy).toEqual({ maxRetries: 2, timeoutMs: 90_000, transport: "stream", progressEverySteps: 6 });

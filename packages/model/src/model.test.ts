...
describe("SdkRunner", () => {
  it("requests a fresh public checkpoint before compacting and excludes failed reads from memory", async () => {
    ...
    const model = new MockLanguageModelV2({ doGenerate: [progressReply("initial"), failedRead, toolReply("read_file", "observed-read"), toolReply("list_dir", "listing"), progressReply("checkpoint"), reply()] });
    const run = input(budget({ maxSteps: 12, maxTokens: 500_000 }));
    const read = vi.fn().mockRejectedValueOnce(new Error("fixture temporarily unavailable")).mockResolvedValueOnce("Current fixture contract.");
    run.tools = [fileTool(read), { name: "list_dir", description: "List fixture paths", schema: z.object({ path: z.string() }), execute: async () => "large-old-fixture-list".repeat(600) }, { name: "report_progress", description: "Publish a public progress update", schema: z.object({ summary: z.string() }), execute: async () => ({ recorded: true }) }];
    ...
    expect(model.doGenerateCalls[4]!.toolChoice).toEqual({ type: "tool", toolName: "report_progress" });
    const continued = model.doGenerateCalls[5]!;
    expect(continued.tools?.map(tool => tool.name)).toEqual(["read_file", "list_dir", "report_progress"]);
    ...
    expect(JSON.stringify(continued.prompt)).toContain("Current fixture contract.");
    expect(JSON.stringify(continued.prompt)).not.toContain("large-old-fixture-list");
    expect(JSON.stringify(continued.prompt)).not.toMatch(/fixture temporarily unavailable|unread.ts/);
